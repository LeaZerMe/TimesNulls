...
import { StyleSheet, Text, View, Alert, FlatList,
  ...
  export default class App extends React.Component {
    ...
    startFade(val) {  
      ...
      setTimeout(() => {this.setState({msgTxt: ""}); this.socket.emit('step', "end"); this.setState({disablePlay: false});}, 3000);
    }
      ...
      if(!this.state.readyPlay) {
        return (
          <View key="main" style={styles.container}>        
            <StartingComponent socket={this.socket} answerName={this.answerName}></StartingComponent>
          </View>
          );
      } else {
...
checkStep(num) {
  ...
  if(this.state.next != this.name || this.state.disablePlay) return;
